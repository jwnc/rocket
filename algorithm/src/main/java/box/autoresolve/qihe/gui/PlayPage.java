...
import box.autoresolve.qihe.command.FindCommand;
...
public class PlayPage extends JFrame implements ActionListener, KeyListener {
    ...
    private void add(int rank) {//删除fenceng中的全部元素，并添加新的元素。
        ...
        for (int i = 0; i < DataStatic.boxNum; i++) {//创建箱子标签
            JLabel boxLabel = new JLabel(new ImageIcon("algorithm/src/main/resources/qihe/box.png"));
        ...
        for (int i = 0; i < DataStatic.kuan; i++) {
            for (int j = 0; j < DataStatic.chang; j++) {
                switch (map[i][j]) {
                    case '#'://墙
                        JLabel qiang = new JLabel(new ImageIcon("algorithm/src/main/resources/qihe/wall.png"));
                        qiang.setBounds(zuo + j * picSize, shang + i * picSize - 10, 35, 45);
                        fenceng.add(qiang, new Integer(10 + i * 2));
                        break;
                    case '.'://终点
                        JLabel zhongdian = new JLabel(new ImageIcon("algorithm/src/main/resources/qihe/yelloball.png"));
